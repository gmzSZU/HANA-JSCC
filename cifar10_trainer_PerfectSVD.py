...
save_path = "./results/CIFAR10_MIMO_" + str(num_antenna_tx) + "x" + \
            str(num_antenna_rx) + "_SwinJSCC_SA_Perfect_SVD.pth"
...
trainset = torchvision.datasets.CIFAR10(root='../Channel_matrix_completion/datasets/', train=True, download=False, 
                                        transform=transform_train)
train_loader = torch.utils.data.DataLoader(trainset, batch_size=train_batch_size, shuffle=True, num_workers=8)
valset = torchvision.datasets.CIFAR10(root='../Channel_matrix_completion/datasets/', train=False, download=False, 
...
with open(filename, "w") as f:
    print("Number of Tx#Rx Antennas: %d#%d" % (num_antenna_tx, num_antenna_rx))
    ...
    f.write("Sub-optimal PSNR: %.5f" % best_psnr)
    f.write("Corresponding Epoch: %d" % best_epoch)
